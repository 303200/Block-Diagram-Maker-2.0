...
export default function Connector({ connector, onSelect, isConnecting }) {
    ...
    return (
        <div onClick={onSelectHandler} className={`${isConnecting ? undefined : styles.container}`} style={connector.style.showXarrow ? { display: "flex" } : { display: "none" }}>
            <Xarrow start={connector.start} end={connector.end}
                ...
                labels={{middle: (
                        <div style={connector.style.middleLabelStyle}>
                            {connector.style.middleLabelText}
                        </div>
                    )
                }}
            />
